...
1. Safari 18以降(iOS 18以降、macOS Sequoia以降)では、Safariの検索エンジンに検索クエリが送信される前に、CSEが介入して送信を防げる可能性があります。この場合、あなたの検索クエリは漏洩しません。
2. Safari 18以降でも、様々な理由でCSEの介入が遅れた場合、Safariの検索エンジンへ検索クエリが送信される可能性があります。
3. Safari 18未満の場合、クエリはほぼ確実に漏洩します。これはCSEがURLを検知してリクエストに介入できるタイミングが、最新のSafariよりも遅くなるためです。
